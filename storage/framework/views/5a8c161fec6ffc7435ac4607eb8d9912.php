<script>
 ...
 $(document).ready(function() {
  $('#userInfoModalCenter').on('show.bs.modal', function(event) {
   ...
   function loadAppointments(selectedClinicId, type) {
        ...
        $.ajax({
            type: "GET",
            url: "<?php echo e(route('dr-appointments')); ?>",
            data: {
 ...
 datepickerSpan.textContent = initialDate;
  ...
  function handleDateSelection(selectedDate, selectedClinicId, filter_text) {
        ...
        $.ajax({
            url: "<?php echo e(route('dr.turn.my-appointments.by-date')); ?>",
            method: 'GET',
            data: {
                date: selectedDate,
                selectedClinicId: selectedClinicId,
...
let selectedCurrentTextDropToggle = "";
 $(document).ready(function() {
  ...
  $('.appointments-filter-drop-toggle li').on('click', function() {
   ...
   $.ajax({
    url: "<?php echo e(route('dr.turn.filter-appointments')); ?>",
    type: "GET",
    data: {
...
<?php /**PATH D:\MyProjects\Benobe\panel\resources\views/dr/panel/turn/schedule/option/appointments.blade.php ENDPATH**/ ?>