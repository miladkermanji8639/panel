...
$(document).ready(function () {
 ...
 $(document).on('click', '#saveSelection', function() {
  ...
  showLoading();
  $.ajax({
   url: "<?php echo e(route('copy-work-hours')); ?>",
   method: 'POST',
   data: {
 ...
 function reloadDayData(day) {
  $.ajax({
   url: "<?php echo e(route('dr-get-work-schedule')); ?>",
   method: 'GET',
   data:{
...
$(document).on('click', '#saveSingleSlotSelection', function() {
    ...
    $.ajax({
        url: "<?php echo e(route('copy-single-slot')); ?>",
        method: 'POST',
        data: {
            source_day: sourceDay,
            target_days: targetDays,
 ...
 $(document).on('click', '[data-target="#scheduleModal"]', function() {
  ...
  $.ajax({
   url: "<?php echo e(route('get-appointment-settings')); ?>",
   method: 'GET',
   data: {
...
$(document).on("click", ".remove-row-btn", function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            if (slotId) {
                $.ajax({
                    url: "<?php echo e(route('appointment.slots.destroy', ':id')); ?>".replace(':id', slotId),
                    method: 'DELETE',
                    data: {
                        _token: '<?php echo e(csrf_token()); ?>',
                        day: day,
  ...
  // تابع ذخیره‌سازی برنامه کاری
   function saveWorkSchedule() {
     const data = {
       ...
       days: {},
       selectedClinicId: localStorage.getItem('selectedClinicId'), // افزودن `selectedClinicId`
     };
     ...
     $.ajax({
       url: "<?php echo e(route('dr-save-work-schedule')); ?>",
       method: 'POST',
       data: JSON.stringify(data), // ارسال داده‌ها به فرمت JSON
       contentType: 'application/json', // تنظیم محتوای ارسال به `application/json`
       headers: {
         'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
       },
       success: function (response) {
         toastr.success('تنظیمات ساعات کاری با موفقیت ذخیره شد.');
         // بررسی وجود `workSchedules` قبل از پردازش
         if (response.workSchedules && Array.isArray(response.workSchedules)) {
  ...
  $(document).ready(function () {
    ...
    $.each(["saturday", "sunday", "monday", "tuesday", "wednesday", "thursday", "friday"], function (index, day) {
      $(`#${day}`).on('change', function () {
        ...
        $.ajax({
          url: "<?php echo e(route('update-work-day-status')); ?>",
          method: 'POST',
          data: {
            day: day,
 ...
 $(document).ready(function() {
  $('#appointment-toggle').on('change', function() {
   ...
   $.ajax({
    url: "<?php echo e(route('update-auto-scheduling')); ?>",
    method: 'POST',
    data: {
...
$(document).on("click", "#saveSelectionCalculator", function() {
    ...
    $.ajax({
        url: "<?php echo e(route('save-time-slot')); ?>",
        method: "POST",
        data: {
            day: day,
            start_time: startTime,
 ...
 function checkAllDaysSettings(day, startTime, endTime, maxAppointments) {
  $.ajax({
   url: "<?php echo e(route('get-all-days-settings')); ?>",
   method: 'GET',
   data: {
    day: day,
    start_time: startTime,
...
$(document).on('click', '#saveSchedule', function() {
    ...
    $.ajax({
        url: "<?php echo e(route('save-appointment-settings')); ?>",
        method: 'POST',
        data: {
...
$(document).on('click', '.delete-schedule-setting', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "<?php echo e(route('delete-schedule-setting')); ?>",
                method: 'POST',
                data: {
                    day: day,
...
<?php /**PATH D:\MyProjects\Benobe\panel\resources\views/dr/panel/my-tools/workhours.blade.php ENDPATH**/ ?>