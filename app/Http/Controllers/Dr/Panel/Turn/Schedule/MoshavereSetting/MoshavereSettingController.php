...
class MoshavereSettingController
{
    ...
    public function workhours()
    {
        ...
        $cacheKey = "doctor_counseling_workhours_{$doctorId}";
        $workHoursData = Cache::remember($cacheKey, 3600, function () use ($doctorId) {
            $counselingConfig = DoctorCounselingConfig::firstOrCreate(
                ['doctor_id' => $doctorId],
                [
                    'auto_scheduling' => true,
                    'online_consultation' => false,
                    ...
                    'calendar_days' => 30,
                    'consultation_types' => json_encode(['general']),
                ]
            );
            $workSchedules = DoctorCounselingWorkSchedule::where('doctor_id', $doctorId)->get();
            return [
                'counselingConfig' => $counselingConfig,
                'workSchedules' => $workSchedules
            ];
        });
        return view("dr.panel.turn.schedule.moshavere_setting.index", [
            'counselingConfig' => $workHoursData['counselingConfig'],
    ...
    public function updateAutoScheduling(Request $request)
    {
        $validated = $request->validate([
            'auto_scheduling' => [
                'required',
                'in:0,1,true,false', // مقادیر معتبر برای تایید ورودی
            ],
        ]);
        // تبدیل مقدار به boolean
        ...
        try {
            $doctor = Auth::guard('doctor')->user();
            // ایجاد یا بروزرسانی تنظیمات در جدول جدید
            ...
            return response()->json([
                ...
                'status' => true,
                'data' => [
                    'auto_scheduling' => $counselingConfig->auto_scheduling
                ]
            ]);
        } catch (\Exception $e) {
    ...
    private function calculateMaxAppointments($startTime, $endTime)
    {
        try {
            ...
            // بازیابی مدت زمان هر نوبت از تنظیمات مشاوره
            $appointmentDuration = DoctorCounselingConfig::where('doctor_id', Auth::guard('doctor')->id())
                ->value('appointment_duration') ?? config('settings.default_appointment_duration', 20);
    ...
    {
        try {
            // پیدا کردن اسلات مشاوره با استفاده از ID
            $counselingSlot = DoctorCounselingSlot::findOrFail($id);
            // حذف اسلات
            ...
            Log::error('خطا در حذف اسلات مشاوره: ' . $e->getMessage(), [
                'slot_id' => $id,
                'trace' => $e->getTraceAsString(),
            ]);
            return response()->json([
                'message' => 'خطا در حذف اسلات مشاوره',
                'status' => false
            ], 500);
        }
    }
