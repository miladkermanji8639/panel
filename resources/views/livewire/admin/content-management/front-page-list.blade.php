    ...
    <div class="container px-0 mb-2">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 bg-light p-3 rounded-3 shadow-sm">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <a href="{{ route('admin.content-management.front-pages.create') }}"
                        ...
                        <div class="card-body p-4 d-flex align-items-center justify-content-between gap-3 flex-wrap">
                            <div class="d-flex align-items-center gap-3">
                                <input type="checkbox" class="form-check-input flex-shrink-0 shadow-sm"
                                    wire:model.live="selectedPages" value="{{ $page->id }}">
                                <div class="flex-grow-1">
                                    <h6 class="mb-1 fw-semibold text-dark">
                                        <i class="ti ti-eye me-2"></i>{{ $page->title }}
                                    </h6>
                                    <small class="text-muted d-block">{{ $page->persian_date }}</small>
                                </div>
                            </div>
                            <div class="d-flex align-items-center gap-3">
                                <div class="form-check form-switch">
                                    <input type="checkbox" class="form-check-input shadow-sm"
                                        wire:model.live="pageStatuses.{{ $page->id }}"
                                        wire:change="toggleStatus({{ $page->id }})" id="status-{{ $page->id }}">
                                    <label class="form-check-label text-muted fw-medium mx-2" for="status-{{ $page->id }}">
                                        {{ $pageStatuses[$page->id] ? 'فعال' : 'غیرفعال' }}
                                    </label>
                                </div>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end shadow-sm rounded-3">
                                        <a class="dropdown-item d-flex align-items-center gap-2"
                                            href="{{ route('admin.content-management.front-pages.edit', $page->id) }}">
