    ...
    <div class="container px-0 mb-2">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 bg-light p-3 rounded-3 shadow-sm">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <a href="{{ route('admin.content-management.links.create') }}"
                        ...
                        <div class="card-body p-4 d-flex align-items-center justify-content-between gap-3 flex-wrap">
                            <div class="d-flex align-items-center gap-3">
                                <input type="checkbox" class="form-check-input flex-shrink-0 shadow-sm"
                                    wire:model.live="selectedLinks" value="{{ $link->id }}">
                                <div class="flex-grow-1">
                                    <h6 class="mb-1 fw-semibold text-dark">{{ $link->name }}</h6>
                                    <small class="text-muted d-block">{{ $link->category->name }}</small>
                                </div>
                            </div>
                            <div class="text-muted mb-0 text-ellipsis flex-grow-1" style="max-width: 40%;">
                                <a href="{{ $link->url }}" target="_blank"
                                    class="text-primary">{{ Str::limit($link->url, 50) }}</a>
                            </div>
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="form-check form-switch">
                                    <input type="checkbox" class="form-check-input shadow-sm"
                                        wire:model.live="linkStatuses.{{ $link->id }}"
                                        wire:change="toggleStatus({{ $link->id }})" id="status-{{ $link->id }}">
                                    <label class="form-check-label text-muted fw-medium mx-2" for="status-{{ $link->id }}">
                                        {{ $linkStatuses[$link->id] ? 'فعال' : 'غیرفعال' }}
                                    </label>
                                </div>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end shadow-sm rounded-3">
                                        <a class="dropdown-item d-flex align-items-center gap-2"
                                            href="{{ route('admin.content-management.links.edit', $link->id) }}">
