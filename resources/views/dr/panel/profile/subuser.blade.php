 ...
 $(document).ready(function() {
  ...
  function showToast(message, type = 'success') {
   Toastify({
    ...
    position: 'right',
    style: {
     background: type === 'success' ? 'green' : 'red' // تغییر به style.background
    }
   }).showToast();
  }
  ...
  function updateSubUserList(subUsers) {
   if (!subUsers || !Array.isArray(subUsers)) {
    console.error("subUsers is undefined or not an array", subUsers);
    return;
   }
   ...
   $.ajax({
    ...
    error: function(xhr) {
     if (xhr.responseJSON && xhr.responseJSON.error) {
      ...
      showToast('خطایی در بروزرسانی رخ داد!', 'error');
     }
    },
    complete: function() {
     buttonText.show();
